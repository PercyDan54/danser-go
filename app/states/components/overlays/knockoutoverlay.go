...
func (overlay *KnockoutOverlay) DrawHUD(batch *batch.QuadBatch, colors []color2.Color, alpha float64) {
	...
	for _, rep := range overlay.playersArray {
		...
		if r.Grade != osu.NONE {
			...
			batch.SetTranslation(vector.NewVec2d(5.2*scl+nWidth+xSlideLeft, rowBaseY))
		...
		if player.lastHit != 0 {
			...
			if tex != "" {
				...
				batch.SetTranslation(vector.NewVec2d(6.2*scl+width+nWidth+scl*(float64(hitTexture.Width)/float64(hitTexture.Height))*0.5+xSlideLeft, rowBaseY))
	...
	for _, rep := range overlay.playersArray {
		...
		overlay.font.DrawOrigin(batch, 4.2*scl+xSlideLeft, rowBaseY, vector.CentreLeft, scl, true, accuracy)
		...
		batch.SetColor(float64(colors[rep.oldIndex].R), float64(colors[rep.oldIndex].G), float64(colors[rep.oldIndex].B), alpha*player.fade.GetValue())
		overlay.font.DrawOrigin(batch, 6*scl+nWidth+xSlideLeft, rowBaseY, vector.CentreLeft, scl, false, r.Name)
		width := overlay.font.GetWidth(scl, r.Name)

		batch.SetColor(1, 1, 1, alpha*player.fade.GetValue())

		if r.Mods != "" {
			overlay.font.DrawOrigin(batch, 6*scl+width+nWidth+xSlideLeft, rowBaseY+ascScl, vector.BottomLeft, scl*0.8, false, "+"+r.Mods)
		}
	}
}
